    ...
    <div className="my-2.5 sm:my-4 rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-2.5 sm:p-4 shadow-xs transition-colors w-full max-w-full min-w-0">
      <div className="flex flex-wrap items-center justify-between gap-1.5 sm:gap-2 mb-2 pb-2 border-b border-neutral-100 dark:border-neutral-800">
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <span className="font-bold text-xs sm:text-sm text-neutral-900 dark:text-white">{name}</span>
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs">
          <span className="px-1.5 sm:px-2 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-800 dark:text-neutral-200 font-mono font-semibold">
            DMG: {damage}
          </span>
          <span className="px-1.5 sm:px-2 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 font-mono font-medium flex items-center gap-1">
      ...
      {showCommands && (
        <div className="mb-2.5 sm:mb-3.5 p-1.5 sm:p-3.5 rounded-lg bg-cyan-50/50 dark:bg-cyan-950/20 border border-cyan-200/80 dark:border-cyan-900/60 animate-in fade-in-50 duration-150 w-full max-w-full min-w-0 overflow-hidden">
          <div className="text-[10px] sm:text-[11px] font-bold text-cyan-900 dark:text-cyan-300 mb-2 flex items-center justify-between gap-2">
            ...
            <span className="text-[9px] sm:text-[10px] text-cyan-700/80 dark:text-cyan-400/80 font-normal shrink-0">
              （横スクロール可能 ➔）
            </span>
          </div>

          {/* 視覚的な矢印・ボタンフロー */}
          <div className="flex sm:flex-wrap items-center gap-1 sm:gap-2 py-1.5 overflow-x-auto max-w-full pb-2.5 touch-pan-x [scrollbar-width:thin] [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-cyan-300 dark:[&::-webkit-scrollbar-thumb]:bg-cyan-700">
            {steps.map((step, idx) => (
              <div key={idx} className="flex items-center gap-1 sm:gap-2 shrink-0">
                <div className="flex items-center gap-1 sm:gap-1.5 px-1.5 py-1 sm:px-2.5 sm:py-1.5 rounded-lg bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 shadow-2xs shrink-0">
                  {/* キャンセル（黄色） */}
                  {step.isCancel && (
