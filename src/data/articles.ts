    ...
    freeContent: {
      ...
      sections: [
        ...
        },
        {
          ...
          body:
            '中距離は、リュウが**「波動拳と通常技を使い分ける間合い」**です。ただし、相手キャラクターのリーチや突進技、弾抜け技の有無によって、有効な中距離の位置は変化します。\n\n中距離では、相手も「前に出たいが波動拳が怖い」「技を振りたいが空振りが怖い」という心理になります。この間合いで機能しやすい技を整理しておきましょう。\n\n・**立ち強P（大P）**：リーチが長く、相手の前進や牽制を止める置き技として機能。パニッシュカウンター時は大ダメージの起点になる\n・**しゃがみ中K（中足）**：一歩踏み込んで下段を意識させる差し込み技。キャンセルドライブラッシュを仕込むことでリターンを出せる\n・**中足刀（中・上段足刀蹴り）**：相手の技の空振りに合わせる差し返し技\n・**電刃錬気**：相手が後退して間合いが離れた際に溜める強化行動\n\n【成立条件の注意点】\n差し返しは、狙う相手の技・空振りする位置・届く距離・相手の硬直が揃って初めて成立します。中足刀が万能な差し返し手段というわけではなく、相手の技の長さに応じて間合いを測る必要があります。\n\nまた、電刃錬気も相手が後退しただけで安全とは限りません。相手の弾抜けSAや生ラッシュが届く距離でないかを確認したうえで溜めましょう。',
        },
        {
          title: '立ち回りの整理枠組み：三すくみの考え方',
          diagramType: 'neutral-triangle',
          body:
            '格闘ゲームの立ち回りを整理する枠組みとして「置き・差し込み・差し返し」の三すくみがあります。\n\nリュウの実戦に当てはめると、以下の関係で整理できます。\n\n・**置き（下中P、立ち強P）**：前に歩いてくる相手やラッシュを止める\n・**差し込み（前歩きしゃがみ中K、ラッシュ攻撃）**：じっと待っている相手の懐に踏み込む\n・**差し返し（立ち強P、中足刀）**：相手が空振った牽制技の隙を殴り返す\n\nただし、これはあくまで状況を整理するためのフレームワークです。実戦では互いの間合い、技のリーチや硬直、入力のタイミングによって結果が大きく変わるため、固定的なじゃんけんとして過信せず、間合い管理の目安として使いましょう。',
        },
        {
